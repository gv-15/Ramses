...
export default class StateChart {
    ...
    init(states = []) {
        ...
        states.forEach((st) => {
            this.states.push(new StateElement(st.name, st.x, st.y, st.isTerminalState, st.isInitialState, st.comments));
        });
        ...
        states.forEach(st => {
                ...
                if (st.transitions === undefined) {
                    ...
                    st.transitions.forEach(tr => {
                        ...
                        } else console.log('estado no encontrado en la conexion ' + st.name + ' to ' + tr.id.split('_')[1]); //TODO mejorar si se quiere
                    })
                }
            })
            // this._redraw();
    ...
    deleteTransition(id) {
            ...
            from.transitions.splice(from.transitions.findIndex(t => t === states[1]), 1);
        }
    ...
    modifyStateData(stId, data) {
        ...
        ['comments', 'isTerminalState', 'isInitialState'].forEach(field => state[field] = data[field]);
        return ('');
    }
    getState(stName) {
    ...
    deleteState(q) {
            this.states.forEach(st => st.transitions = st.transitions.filter(t => t.to !== q)); //solo las que no apuntaban
            this.states.splice(this.states.findIndex(st => st.name === q), 1);
        }
